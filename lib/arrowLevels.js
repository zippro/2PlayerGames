...
// The arrow escapes by extending its tip outward while the tail retracts
// along the arrow's own path. Only the cells AHEAD of the tip in the
// escape direction need to be clear of other arrows.
export function canArrowEscape(arrow, allArrows, gridW, gridH) {
  ...
  // Check from tip outward in escape direction
  let sx = tip[0] + dx;
  let sy = tip[1] + dy;
  while (sx >= 0 && sy >= 0 && sx < gridW && sy < gridH) {
    if (otherCells.has(`${sx},${sy}`)) {
      return false;
    }
    sx += dx;
    sy += dy;
...
// Checks from the tip outward in escape direction, returns distance to first hit.
