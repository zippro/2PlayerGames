...
export const sounds = {
  ...
  /** Ignis — soft fire crackle for arrow extraction trail */
  ignis() {
    if (!isEnabled()) return;
    try {
      const ctx = getCtx();
      ...
      const data = buffer.getChannelData(0);
      for (let i = 0; i < bufferSize; i++) {
        // Sparse crackle — mostly silence with soft pops
        data[i] = Math.random() > 0.97 ? (Math.random() - 0.5) * 0.4 : (Math.random() - 0.5) * 0.02;
      }
      const source = ctx.createBufferSource();
      source.buffer = buffer;
      // Low-pass for warmth
      ...
      gain.gain.setValueAtTime(0.06, ctx.currentTime + 0.4);
